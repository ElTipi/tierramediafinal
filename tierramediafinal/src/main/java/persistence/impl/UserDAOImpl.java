...
public class UserDAOImpl implements UserDAO {
	...
	public int insert(User user) {
		try {
			String sql = "INSERT INTO USERS (USERNAME, PASSWORD, COINS, TIME, PREFERENCIA) VALUES (?, ?, ?, ?, ?)";
			...
			statement.setString(5, user.getPreferencia()); //AGREGADO
	...
	public int update(User user) {
		try {
			String sql = "UPDATE USERS SET COINS = ?, TIME = ?, USERNAME = ?, PASSWORD=?, PREFERENCIA=? WHERE ID = ?";
			Connection conn = ConnectionProvider.getConnection();
	...
	public User findByUsername(String username) {
		try {
			String sql = "SELECT * FROM USERS WHERE USERNAME = ? AND BORRADO=0";
			Connection conn = ConnectionProvider.getConnection();
			PreparedStatement statement = conn.prepareStatement(sql);
	...
	public User find(Integer id) {
		try {
			String sql = "SELECT * FROM USERS WHERE ID = ? AND BORRADO=0";
			Connection conn = ConnectionProvider.getConnection();
			PreparedStatement statement = conn.prepareStatement(sql);
	...
	private User toUser(ResultSet userRegister) throws SQLException {
		return new User(userRegister.getInt(1), userRegister.getString(2), userRegister.getString(3),
				userRegister.getInt(5), userRegister.getDouble(6), userRegister.getString(7), userRegister.getBoolean(4));
	}
