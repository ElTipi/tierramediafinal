...
<body>
	...
	<main class="container">
		...
		<c:if test="${user.isAdmin()}">
			<div class="mb-3">
				<a href="/tierramediafinal/users/create.do" class="btn btn-primary"
				...
				<c:forEach items="${users}" var="tmp_user">
					<tr>
						...
						<td><c:if test="${user.admin && (!tmp_user.admin || tmp_user.id == user.id)}">
								<a href="/tierramediafinal/users/edit.do?id=${tmp_user.id}"
									class="btn btn-light rounded-0" role="button"><i
									class="bi bi-pencil-fill"></i></a>
								<a href="/tierramediafinal/users/delete.do?id=${tmp_user.id}"
